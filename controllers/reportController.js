  ...
  static async postReport(req, res, next) {
    try {
      ...
      if (file) {
        ...
        const report = await Report.create({ name, from, age, description, long, lat, photo: photo });
      } else {
        const report = await Report.create({ name, from, age, description, long, lat, photo: "" });
      }
